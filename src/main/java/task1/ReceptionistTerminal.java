...
public class ReceptionistTerminal extends Terminal {
	...
	private final static List<String> commands = Arrays.asList("show-hotels", "show-rooms", "add-reservation",
			"show-reservations", "update-reservation", "delete-reservation", "set-room", "register", "check-in",
			"check-out", "sim-key-value", "help", "logout");
	...
	private void addReservation(String[] options) {
		try {
			...
			Reservation reservation = new Reservation(null, from, to, null);
			Application.hotelDatabaseManager.insertReservation(hotelId, roomNumber, username, reservation);
	...
	private void updateReservation(String[] options) {
		try {
			...
			if (!cmd.hasOption("hotel") && !cmd.hasOption("room") && !cmd.hasOption("customer")
					&& !cmd.hasOption("from") && !cmd.hasOption("to"))
				throw new ParseException("Provide at least one new value to update the reservation");

			// get reservation to modify
			Reservation oldReservation = Application.hotelDatabaseManager.readReservation(oldHotelId, oldRoomNumber,
					oldCheckIn);

			// get values of optional parameters
			long newHotelId = cmd.hasOption("hotel") ? ((Number) cmd.getParsedOptionValue("hotel")).longValue()
					: oldReservation.getRoom().getHotel().getId();
			int newRoomNumber = cmd.hasOption("room") ? ((Number) cmd.getParsedOptionValue("room")).intValue()
					: oldReservation.getRoom().getNumber();
			String newUsername = cmd.hasOption("customer") ? cmd.getOptionValue("customer")
					: oldReservation.getCustomer().getUsername();
			Date newCheckIn = cmd.hasOption("from") ? parseDate(cmd.getOptionValue("from"))
					: oldReservation.getCheckInDate();
			Date newCheckOut = cmd.hasOption("to") ? parseDate(cmd.getOptionValue("to"))
			...
			List<Room> reservableRooms = Application.hotelDatabaseManager.getReservableRooms(newRoom.getHotel(),
